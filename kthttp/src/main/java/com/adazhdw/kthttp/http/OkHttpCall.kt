...
import com.adazhdw.kthttp.http.Utils.Companion.throwIfFatal
...
class OkHttpCall<T>(
    ...
    private val responseConverter: Converter<okhttp3.ResponseBody, T>
) : Call<T> {
    ...
    override fun copy(): Call<T> {
        return OkHttpCall<T>(requestFactory, callFactory, responseConverter)
    }
    ...
    private fun parseResponse(rawResponse: okhttp3.Response): Response<T> {
        rawResponse.use { response ->
            ...
            try {
                val body: T? = responseConverter.convert(catchingBody)
