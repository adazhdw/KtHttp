...
class ProgressRequestBody(
    ...
    private val progress: ((value: Float, total: Long) -> Unit)? = null
) : RequestBody() {
    //包装完成的BufferedSink
    ...
    private fun sink(sink: BufferedSink): Sink {
        return object : ForwardingSink(sink) {
            ...
            override fun write(source: Buffer, byteCount: Long) {
                ...
                val progress1 = bytesWritten * 1.0f / contentLength
                KtExecutors.mainThread.execute { progress?.invoke(progress1, contentLength) }
            }
        }
    }
}